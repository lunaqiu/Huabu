...
The generic `fixed ↔ auto` flip is reachable from the node toolbar and the multi-select toolbar. The chevron a note draws along its bottom edge has stricter semantics: expand hands height to the renderer, while collapse always pins `collapsedLayoutHeight` and never restores a remembered user height or seeds from a pending measurement. That chevron was previously inert, and for a reason worth preserving: as a click target it spanned the card's full width right where selection and resize gestures land, so it fired by accident far more often than on purpose. It is now a control again, but a narrow one — the full-width container stays `pointer-events-none` and only a 24×24 button opts back in, carrying `nodrag nopan` so React Flow cannot read the press as the start of a drag or pan. Without a reachable expand affordance the collapse-on-create rule in §8 would be a trap rather than a default.

The chevron renders when a fixed note is truncated (pointing down: expand) and also on a selected, already-expanded note whose content is past the collapse threshold (pointing up: collapse). A temporarily truncated auto note renders only the fade while its measurement is pending; it must not offer collapse, because pinning the preview height can enlarge a short note. The second interactive case exists because an expanded note hides nothing, so truncation alone would leave no way back.
